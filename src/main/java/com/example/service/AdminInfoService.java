...
public class AdminInfoService {
    ...
    public AdminInfo add(AdminInfo adminInfo) {
        // 唯一校验
        int count = adminInfoDao.checkRepeat("name", adminInfo.getName(), null);//检验是否重复
        if (count > 0) {
            throw new CustomException("1001", "用户名\"" + adminInfo.getName() + "\"已存在");//如果count大于0说明该用户存在,抛出异常
        }
        if (StringUtils.isEmpty(adminInfo.getPassword())) {//若密码为空那就设置密码为123456,并对他进行加密
            // 默认密码123456
